      ...
      <main className="pt-16 p-7">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mb-6">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="px-6 py-5">
              <form onSubmit={handleEnroll} className="space-y-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Select Programme</label>
                  <select required value={selected} onChange={e => setSelected(e.target.value)}
                    className="w-full px-4 py-2.5 border-2 border-gray-200 rounded-xl text-sm focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-100 transition-all bg-white">
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
            ...
            <div className="px-6 py-5 flex flex-col items-center justify-center min-h-[200px]">
              {student?.course ? (
                <div className="text-center">
                  <div className="w-16 h-16 bg-blue-50 rounded-2xl flex items-center justify-center mx-auto mb-4">
                    <BookOpen size={28} className="text-blue-600" />
                  </div>
                  <p className="font-bold text-gray-900 text-base mb-3">{student.course}</p>
                  ...
                  <div className="mt-5 pt-4 border-t border-gray-100 w-full space-y-2">
        ...
        <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          <div className="p-6 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
            {PROGRAMMES.map(p => {
              ...
              return (
                <div key={p} onClick={() => setSelected(p)} className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 cursor-pointer transition-all
                  ${active ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-blue-300 hover:bg-gray-50'}`}>
                  <BookOpen size={15} className={active ? 'text-blue-600' : 'text-gray-300'} />
                  <span className={`text-sm ${active ? 'font-bold text-blue-700' : 'font-medium text-gray-600'}`}>{p}</span>
                  {active && <span className="ml-auto text-[10px] font-bold bg-blue-100 text-blue-700 px-2 py-0.5 rounded-full">Enrolled</span>}
                </div>
              )
            })}
          </div>
